...
def second_level_one_sample_ttest(
    ...
    **threshold_kwargs,
    ...
    resolved_mask_img = None
    if mask_image is not None:
        if isinstance(mask_image, str) and mask_image.lower() in ("mni", "mni_template", "mni152"):
    ...
    if inference == "parametric":
        ...
        thresholded_two_sided_path = group_dir / f"tmap_{height_control}_k{int(cluster_threshold)}.nii.gz"
        thresholded_two_sided_map = _threshold_and_cache(
            t_scores_img, out_path=thresholded_two_sided_path, two_sided=True, mask_img=resolved_mask_img
        )
        thresholded_gt_path = group_dir / f"tmap_{height_control}_k{int(cluster_threshold)}_gt_onesided.nii.gz"
        thresholded_gt_map = _threshold_and_cache(
            t_scores_img, out_path=thresholded_gt_path, two_sided=False, mask_img=gt_mask_img
        )
        t_flipped = image.math_img("-img", img=t_scores_img)
        thresholded_lt_path = group_dir / f"tmap_{height_control}_k{int(cluster_threshold)}_lt_onesided.nii.gz"
        thresholded_lt_map = _threshold_and_cache(
            t_flipped, out_path=thresholded_lt_path, two_sided=False, mask_img=lt_mask_img
        )
        base_stem = f"tmap_{height_control}_k{int(cluster_threshold)}"
        base_title = f"t-map ({height_control}, k>={int(cluster_threshold)})"
        gt_title = f"t-map > 0 (one-sided, {height_control}, k>={int(cluster_threshold)})"
        lt_title = f"t-map < 0 (one-sided, {height_control}, k>={int(cluster_threshold)})"
    else:
        ...
        def _extract_logp_and_t(ret):
            if isinstance(ret, dict):
                if "logp_max_t" in ret and "t" in ret:
                    return ret["logp_max_t"], ret["t"]
                raise ValueError("Unsupported non_parametric_inference outputs; expected keys 't' and 'logp_max_t'.")
            return ret, None
        ...
        logp_two_img, t_two_img = _extract_logp_and_t(logp_two_ret)
        ...
        thresholded_two_sided_path = group_dir / f"tmap_vfwe_alpha{alpha_tag}.nii.gz"
        thresholded_two_sided_map.to_filename(str(thresholded_two_sided_path))

        # gt (one-sided)
        logp_gt_ret = _np_logp(False, [1.0])
        logp_gt_img, t_gt_img = _extract_logp_and_t(logp_gt_ret)
        if t_gt_img is None:
        ...
        thresholded_gt_path = group_dir / f"tmap_vfwe_alpha{alpha_tag}_gt_onesided.nii.gz"
        ...
        logp_lt_img, t_lt_img = _extract_logp_and_t(logp_lt_ret)
        if t_lt_img is None:
        ...
        thresholded_lt_path = group_dir / f"tmap_vfwe_alpha{alpha_tag}_lt_onesided.nii.gz"
        thresholded_lt_map.to_filename(str(thresholded_lt_path))

        base_stem = f"tmap_vfwe_alpha{alpha_tag}"
...
# Backwards-compatible alias (older name implied MNI, but this function is general)
second_level_group_ttest_mni = second_level_one_sample_ttest
...
def second_level(
    ...
    inference: str = "parametric",
    smoothing_fwhm: Optional[float] = None,
    template_image: Optional[Union[str, Path]] = None,
    ...
    """
    Fire CLI wrapper for :func:`second_level_one_sample_ttest`.
    """
    return second_level_one_sample_ttest(
        maps_dir=maps_dir,
        inference=inference,
        smoothing_fwhm=smoothing_fwhm,
...
def main():
    ...
    fire.Fire({"second_level": second_level})
